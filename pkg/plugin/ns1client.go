...
var (
	errAuthorizationDenied = errors.New("invalid NS1 API key")
...
// Structs matching the response
type PulsarJobValue struct {
	JobID string  `json:"job_id"`
	Value float64 `json:"value"`
}

type PulsarDataPoint struct {
	Timestamp int64            `json:"timestamp"`
	Jobs      []PulsarJobValue `json:"jobs"`
}

type PulsarAPIResponse struct {
	Area    string            `json:"area"`
	Data    []PulsarDataPoint `json:"data"`
...
func (pc *PulsarClient) GetData(apiKey string, query *queryModel) ([]time.Time, []float64, error) {
	...
	for i := offset; i < totalSize; i++ {
		...
		times[idx] = time.Unix(dataPoint.Timestamp, 0)
		// Find the job value for the requested job ID
		for _, job := range dataPoint.Jobs {
			if job.JobID == query.JobID {
				values[idx] = job.Value
				break
			}
		}
		idx++
